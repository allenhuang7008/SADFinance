...
def objective(trial, norm_train, norm_val, trend):
    ...
    val_loss, _ = train_model(params, norm_train, norm_val, trend=trend, n_epochs=10) # here we don't need the optimal state

    return val_loss

def tune_model(norm_train, norm_val, trend):
